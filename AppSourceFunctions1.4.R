...
summary(survfit(train.fit.age, data.frame(
  comb.cont = c(seq(0, 1, 0.1), seq(0, 1, 0.1)),
  age.comb = c(rep("TRUE", 11), rep("FALSE", 11))
)), time = 5) -> y
...
df3.y <- data.frame(
  pred = c(seq(0, 1, 0.1), seq(0, 1, 0.1)),
...
survivalcurveplotPERC <- function(new.sample.meta.score, indexRow) {
  ...
  # new.sample.meta.score <- logistic
  # indexRow <- ROW
  ...
  df.lines.hor$perc <-
    # paste0(round(df.lines.hor$xend / length(y2) * 100), "th")
    paste0(round(df.lines.ver$yend / 1 * 100), "%")
...
SurvivalAgePlot <- function (new.sample.meta.score, indexRow) {
  ...
  df3$pred -> pred
  df3$surv -> surv
  df4 <- subset(df3, age == TRUE)
  df5 <- subset(df3, age != TRUE)
  df4$pred -> pred4
  df4$surv -> surv4
  df5$pred -> pred5
  df5$surv -> surv5
